...
var MAX_SESSION_AGE_MS = 5 * 60 * 1e3;
var KV_SESSION_PREFIX = "session:";
...
var cleanupExpiredSessions = /* @__PURE__ */ __name(() => {
...
var parseSessionPath = /* @__PURE__ */ __name((pathname) => pathname.split("/").filter(Boolean), "parseSessionPath");
var getKvStore = /* @__PURE__ */ __name((env) => env?.SIGNALING_SESSIONS || null, "getKvStore");
var getKvSessionKey = /* @__PURE__ */ __name((sessionId) => `${KV_SESSION_PREFIX}${sessionId}`, "getKvSessionKey");
var saveSession = /* @__PURE__ */ __name(async (sessionId, session, env) => {
  const kv = getKvStore(env);
  if (kv) {
    await kv.put(getKvSessionKey(sessionId), JSON.stringify(session), {
      expirationTtl: Math.ceil(MAX_SESSION_AGE_MS / 1e3)
    });
    return;
  }
  sessions.set(sessionId, session);
}, "saveSession");
var loadSession = /* @__PURE__ */ __name(async (sessionId, env) => {
  const kv = getKvStore(env);
  if (kv) {
...
var deleteSession = /* @__PURE__ */ __name(async (sessionId, env) => {
  const kv = getKvStore(env);
  if (kv) {
...
var getSessionOrError = /* @__PURE__ */ __name(async (sessionId, key, env) => {
...
var handleSignalingRequest = /* @__PURE__ */ __name(async (request, env = {}) => {
...
// api/[[route]].js
var onRequest = /* @__PURE__ */ __name(async (context) => handleSignalingRequest(context.request, context.env), "onRequest");

// ../.wrangler/tmp/pages-mZIbRo/functionsRoutes-0.5815832708713663.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
